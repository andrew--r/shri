...
export default class Door2 extends DoorBase {
	constructor(number, onUnlock) {
		...
		this._tryToInserKey = (function tryToInsertKey() {
			...
			this.keysInLock = this.keysInLock.reduce((keys, key) => {
				if (key.currentColor === lock.currentColor) {
		...
		this.items = {
			...
			keys: ['red', 'green', 'blue'].map((currentColor, index) => new Key({
				currentColor,
				size: keySize,
				defaultPosition: new Vector2d(
	...
	initializeKeys() {
		...
		function keyDragStart(event) {
			...
			const key = self.items.keys.filter((item) => {
				return item.id === parseInt(event.target.getAttribute('data-id'), 10);
			})[0];
	...
	initializeLock() {
		...
		setInterval(() => {
			lock.switchColor(colorsTable[lock.currentColor]);
			lock.render();
		}, 150);
	}
}